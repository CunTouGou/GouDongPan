...
import com.hgz.file.helper.DocumentManager;
import com.hgz.file.helper.FileUtility;
import com.hgz.file.helper.ServiceConverter;
import lombok.Data;
...
public class FileModel {
    ...
    public FileModel(String fileName, String fileUrl, String fileModifyTime, String uid, String uname, String mode) {
        ...
        document.info.favorite = uid != null && !uid.isEmpty() ? "uid-2".equals(uid) : null;
        ...
        if (uname != null) {
            editorConfig.user.name = "uid-0".equals(uid) ? null : uname;
        }
        if ("uid-2".equals(editorConfig.user.id)) {
            editorConfig.user.group = "group-2";
        }
        if ("uid-3".equals(editorConfig.user.id)) {
            editorConfig.user.group = "group-3";
        }
...
//        editorConfig.customization.goback.url = DocumentManager.GetServerUrl(false) + "/IndexServlet";
    ...
    public void changeType(String _mode, String _type) {
        ...
        editorConfig.customization.submitForm = canEdit && ("edit".equals(mode) || "fillForms".equals(mode));
    ...
    public class Permissions {
        ...
        public Permissions(String mode, String type, Boolean canEdit) {
            ...
            download = true;
            edit = canEdit && ("edit".equals(mode) || "view".equals(mode) || "filter".equals(mode) || "blockcontent".equals(mode));
