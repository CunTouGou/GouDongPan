...
public class ShareController {
    ...
    public RestResult<ShareFileVO> shareFile(@RequestBody ShareFileDTO shareSecretDTO) {
        ...
        String uuid = RandomUtil.randomString(
        ...
        for (ShareFile shareFile : fileList) {
            ...
            if (userFile.getIsDir() == 1) {
                ...
                List<UserFile> userfileList = userFileService.selectUserFileByLikeRightFilePath(gouDongFile.getPath(), sessionUserBean.getUserId());
                for (UserFile userFile1 : userfileList) {
                    ...
                    shareFile1.setShareFilePath(userFile1.getFilePath().replaceFirst("/".equals(userFile.getFilePath()) ? "" : userFile.getFilePath(), ""));
    ...
    public RestResult saveShareFile(@RequestBody SaveShareFileDTO saveShareFileDTO) {
        ...
        for (ShareFile shareFile : fileList) {
            ...
            if (userFile.getIsDir() == 1) {
                List<UserFile> userfileList = userFileService.selectUserFileByLikeRightFilePath(new GouDongFile(userFile.getFilePath(), userFile.getFileName(), true).getPath(), userFile.getUserId());
