...
import com.hgz.file.vo.file.SearchFileVO;
...
public class FileController {
    ...
    public RestResult<String> createFile(@Valid @RequestBody CreateFileDTO createFileDto) {
        ...
        UserFile userFile = GouDongFileUtil.getGouDongDir(sessionUserBean.getUserId(), createFileDto.getFilePath(), createFileDto.getFileName());
    ...
    public RestResult<String> renameFile(@RequestBody RenameFileDTO renameFileDto) {
        ...
        if (1 == userFile.getIsDir()) {
            List<UserFile> list = userFileService.selectUserFileByLikeRightFilePath(new GouDongFile(userFile.getFilePath(), userFile.getFileName(), true).getPath(), sessionUserBean.getUserId());

            for (UserFile newUserFile : list) {
                newUserFile.setFilePath(newUserFile.getFilePath().replaceFirst(new GouDongFile(userFile.getFilePath(), userFile.getFileName(), userFile.getIsDir() == 1).getPath(),
                        new GouDongFile(userFile.getFilePath(), renameFileDto.getFileName(), userFile.getIsDir() == 1).getPath()));
    ...
    public RestResult<String> copyFile(@RequestBody CopyFileDTO copyFileDTO) {
        ...
        if (userFile.getIsDir() == 1) {
            GouDongFile gouDongFile = new GouDongFile(oldfilePath, fileName, true);
            if (newfilePath.startsWith(gouDongFile.getPath() + GouDongFile.separator) || newfilePath.equals(gouDongFile.getPath())) {
    ...
    public RestResult<String> batchMoveFile(@RequestBody BatchMoveFileDTO batchMoveFileDto) {
        ...
        for (UserFile userFile : fileList) {
            ...
            if (StrUtil.isEmpty(userFile.getExtendName())) {
    ...
    public RestResult<TreeNode> getFileTree() {
        ...
        resultTreeNode.setLabel(GouDongFile.separator);
        resultTreeNode.setId(0L);
        long id = 1;
        for (int i = 0; i < userFileList.size(); i++) {
            UserFile userFile = userFileList.get(i);
            GouDongFile gouDongFile = new GouDongFile(userFile.getFilePath(), userFile.getFileName(), false);
            String filePath = gouDongFile.getPath();

            Queue<String> queue = new LinkedList<>();

            String[] strArr = filePath.split(GouDongFile.separator);
            ...
            resultTreeNode = fileDealComp.insertTreeNode(resultTreeNode, id++, GouDongFile.separator, queue);
    ...
    public RestResult<String> updateFile(@RequestBody UpdateFileDTO updateFileDTO) {
        ...
        try {
            ...
            throw new GouDongException(999999, "修改文件异常");
        } finally {
            try {
