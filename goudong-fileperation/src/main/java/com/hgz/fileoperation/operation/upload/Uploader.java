package com.hgz.fileoperation.operation.upload;
...
public abstract class Uploader {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                for (MultipartFile multipartFile : multipartFileList) {
                    GouDongMultipartFile gouDongMultipartFile = new GouDongMultipartFile(multipartFile);
                    UploadFileResult uploadFileResult = doUploadFlow(gouDongMultipartFile, uploadFile);
    ...
    protected UploadFileResult doUploadFlow(GouDongMultipartFile gouDongMultipartFile, UploadFile uploadFile) {
        ...
        try {
            rectifier(gouDongMultipartFile, uploadFile);
            uploadFileResult = organizationalResults(gouDongMultipartFile, uploadFile);
        } catch (Exception e) {
            throw new UploadException(e);
        }
    ...
    protected abstract void doUploadFileChunk(GouDongMultipartFile gouDongMultipartFile, UploadFile uploadFile)  throws IOException;
    protected abstract UploadFileResult organizationalResults(GouDongMultipartFile gouDongMultipartFile, UploadFile uploadFile);

    private void rectifier(GouDongMultipartFile gouDongMultipartFile, UploadFile uploadFile) {
        String key = "GouDongUploader:Identifier:" + uploadFile.getIdentifier() + ":lock";
        String current_upload_chunk_number = "GouDongUploader:Identifier:" + uploadFile.getIdentifier() + ":current_upload_chunk_number";
        ...
        try {
            ...
            log.info("文件名{},正在上传第{}块, 共{}块>>>>>>>>>>", gouDongMultipartFile.getMultipartFile().getOriginalFilename(),uploadFile.getChunkNumber(), uploadFile.getTotalChunks());
            if (uploadFile.getChunkNumber() == currentUploadChunkNumber) {
                doUploadFileChunk(gouDongMultipartFile, uploadFile);
                log.info("文件名{},第{}块上传成功", gouDongMultipartFile.getMultipartFile().getOriginalFilename(), uploadFile.getChunkNumber());
                this.redisUtil.getIncr("GouDongUploader:Identifier:" + uploadFile.getIdentifier() + ":current_upload_chunk_number");
            }
        } catch (Exception e) {
