...
public class FileOperationUtils {
    ...
    public static File getLocalSaveFile(String fileUrl) {
        String localSavePath = FileOperationUtils.getStaticPath() + fileUrl;
        return new File(localSavePath);
    }

    public static File getCacheFile(String fileUrl) {
        String cachePath = FileOperationUtils.getStaticPath() + "cache" + File.separator + fileUrl;

        return new File(cachePath);
    }

    public static File getTempFile(String fileUrl) {
        String tempPath = FileOperationUtils.getStaticPath() + "temp" + File.separator + fileUrl;
        File tempFile = new File(tempPath);
        File parentFile = tempFile.getParentFile();
        if (!parentFile.exists()) {
            boolean result = parentFile.mkdirs();
            if (!result) {
                throw new FileOperationException("创建temp目录失败：目录路径："+ parentFile.getPath());
            }
        }
    ...
    public static File getProcessFile(String fileUrl) {
        String processPath = FileOperationUtils.getStaticPath() + "temp" + File.separator + "process" + File.separator + fileUrl;
        File processFile = new File(processPath);
        File parentFile = processFile.getParentFile();
        if (!parentFile.exists()) {
            boolean result = parentFile.mkdirs();
            if (!result) {
                throw new FileOperationException("创建process目录失败：目录路径："+ parentFile.getPath());
            }
        }
        return processFile;
    }
    ...
    public static String getUploadFileUrl(String identifier, String extendName) {
        ...
        File dir = new File(FileOperationUtils.getStaticPath() + path);

        if (!dir.exists()) {

            boolean result = dir.mkdirs();
            if (!result) {
                throw new FileOperationException("创建upload目录失败：目录路径："+ dir.getPath());
            }
